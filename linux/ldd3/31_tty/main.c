...
static unsigned int ldd_tty_write_room(struct tty_struct *tty) {
    int rv;
    unsigned long flags;
    struct ldd_tty_dev *ldd_dev = tty->driver_data;
...
static int __init m_init(void) {
    ...
    ldd_tty_driver = tty_alloc_driver(
        LDD_TTY_MINOR_NR, TTY_DRIVER_RESET_TERMIOS | TTY_DRIVER_REAL_RAW);
    if (IS_ERR(ldd_tty_driver))
        return PTR_ERR(ldd_tty_driver);

    ldd_dev_table =
        kcalloc(LDD_TTY_MINOR_NR, sizeof(struct ldd_tty_dev), GFP_KERNEL);
    if (!ldd_dev_table) {
        tty_driver_kref_put(ldd_tty_driver);
        return -ENOMEM;
    }
