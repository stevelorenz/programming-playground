...
MODULE_PARM_DESC(
	...
	" required cleanup! so, after removal, any op on the debugfs files will "
	"cause"
...
#define MAXUPASS 256  // careful- the kernel stack is small!
	...
	snprintf(locbuf, MAXUPASS - 1,
			 ...
			 "oursecret:%s\n",
			 OURMODNAME, data->tx, data->rx, data->err, data->myword,
			 data->power, data->config1, data->config2, data->config3,
			 data->config3, data->oursecret);
...
#define DBGFS_FILE1 "llkd_dbgfs_show_drvctx"
	file1 = debugfs_create_file(DBGFS_FILE1, 0440, gparent, (void *)gdrvctx,
								&dbgfs_drvctx_fops);
	if (!file1) {
...
static void debugfs_simple_intf_cleanup(void) {
	kfree(gdrvctx);
	if (!cause_an_oops) debugfs_remove_recursive(gparent);
	pr_info("removed\n");
}
