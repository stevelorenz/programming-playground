/* NOTE: I can't compile this successfully on 5.15 kernel.
 * I think it's because of API changes for procfs...
 * */
...
MODULE_DESCRIPTION(
	"LKP-2 book:user_kernel_comm/procfs_simple_intf: simple procfs interfacing "
	"demo");
...
static struct proc_ops proc_show_drvctx(struct seq_file *seq, void *v) {
	if (mutex_lock_interruptible(&mtx)) return -ERESTARTSYS;
	seq_printf(seq,
			   "prodname:%s\n"
...
static struct drv_ctx *alloc_init_drvctx(void) {
	...
	if (!drvctx) return ERR_PTR(-ENOMEM);
...
static ssize_t myproc_write_debug_level(struct file *filp,
	...
	if (debug_level < DEBUG_LEVEL_MIN || debug_level > DEBUG_LEVEL_MAX) {
		pr_info(
			"trying to set invalid value for debug_level\n"
			" [allowed range: %d-%d]\n",
