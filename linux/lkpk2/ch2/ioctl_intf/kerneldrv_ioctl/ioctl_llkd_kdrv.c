...
static int ioctl_intf_major, power = 1; /* 'powered on' by default */
...
static long ioctl_intf_ioctl(struct file *filp, unsigned int cmd,
							 unsigned long arg)
#else
static int ioctl_intf_ioctl(struct inode *ino, struct file *filp,
...
#endif
{
	...
	switch (cmd) {
		...
		case IOCTL_LLKD_IOCQPOWER: /* Get: arg is pointer to result */
			 ...
			 * here, imagine we've done that and placed it into a variable
			 * 'power'
			 */
...
static int __init ioctl_llkd_kdrv_init(void) {
	 ...
	 */
	result =
		register_chrdev(ioctl_intf_major, OURMODNAME, &ioctl_intf_open_fops);
	if (result < 0) {
	...
	if (ioctl_intf_major == 0) ioctl_intf_major = result; /* dynamic */
