...
struct drv_ctx {
	...
	u64 config3;
#define MAXBYTES                                            \
	128 /* Must match the userspace app; we should actually \
...
static ssize_t read_miscdrv_rdwr(struct file *filp, char __user *ubuf,
	...
	if (count < MAXBYTES) {
		dev_warn(
			dev,
			"request # of bytes (%zd) is < required size (%d), aborting read\n",
			count, MAXBYTES);
		goto out_notok;
	}
	if (secret_len <= 0) {
	...
	if (unlikely(!kbuf)) goto out_nomem;
...
/*
 * write_miscdrv_rdwr()
 * The driver's write 'method'; it has effectively 'taken over' the write
 * syscall functionality! The POSIX standard requires that the read() and
 * write() system calls return the number of bytes read or written on success, 0
 * on EOF and -1 (-ve errno) on failure; Here, we accept the string passed to us
 * and update our 'secret' value to it.
 */
static ssize_t write_miscdrv_rdwr(struct file *filp, const char __user *ubuf,
	...
	if (unlikely(count > MAXBYTES)) { /* paranoia */
		dev_warn(dev,
				 "count %zd exceeds max # of bytes allowed, "
				 "aborting write\n",
				 count);
		goto out_nomem;
	}
...
static struct miscdevice llkd_miscdev = {
	...
	.name = "bad_miscdrv",		  /* when misc_register() is invoked, the kernel
								   * will auto-create device file as
								   * /dev/llkd_miscdrv_rdwr;	    also populated within
								   * /sys/class/misc/ and /sys/devices/virtual/misc/ */
...
static int __init bad_miscdrv_init(void) {
	...
	dev_info(dev,
			 "LLKD 'bad' misc driver (major # 10) registered, minor# = %d\n",
			 llkd_miscdev.minor);
	...
	dev_dbg(ctx->dev,
			"A sample print via the dev_dbg(): (bad) driver initialized\n");
