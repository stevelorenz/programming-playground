...
void show_phy_pages(const void *kaddr, size_t len, bool contiguity_check) {
	...
	for (i = 0; i < loops; i++) {
		...
		pr_info("%05d  0x%px   %pa   %ld\n", i, vaddr + (i * PAGE_SIZE), &pa,
				pfn);
		if (!!contiguity_check) prev_pfn = pfn;
	}
}
...
u64 powerof(int base, int exponent) {
	...
	if (base == 0)	// 0^e = 0
		return 0;
	if (base <= 0 || exponent < 0) return -1UL;
	if (exponent == 0)	// b^0 = 1
		return 1;
...
void show_sizeof(void) {
	...
	pr_info(
		...
		sizeof(char), sizeof(short int), sizeof(int), sizeof(long),
		sizeof(long long), sizeof(void *), sizeof(float), sizeof(double),
		sizeof(long double));
}
