...
#define DBGPRINT(string, args...) trace_printk(string, ##args);
#else
 ...
 *
 * (of course, above, we don't display the 'Duration' and 'Function Calls'
 * fields)
 */
...
#define PRINT_CTX()                                                            \
	do {                                                                       \
		...
		if (PRINTCTX_SHOWHDR == 1)                                             \
			pr_debug(                                                          \
				"CPU)  task_name:PID  | "                                      \
				"irqs,need-resched,hard/softirq,preempt-depth  /* "            \
				"func_name() */\n");                                           \
		pr_debug(                                                              \
			"%03d) %c%s%c:%d   |  "                                            \
			"%c%c%c%u   "                                                      \
			"/* %s() */\n",                                                    \
			raw_smp_processor_id(), (!current->mm ? '[' : ' '), current->comm, \
			(!current->mm ? ']' : ' '), current->pid,                          \
			(irqs_disabled() ? 'd' : '.'), (need_resched() ? 'N' : '.'), intr, \
