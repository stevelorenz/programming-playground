...
void *client_handler(void *arg) {
	...
	calc_proto_ser_constructor(context.ser, &context, 256);
	...
	calc_service_constructor(context.svc);
	...
	char read_buf[128];
	while (1) {
		int ret = read(context.addr->sd, read_buf, 128);
		if (ret == 0 || ret == -1) {
			break;
		}
		struct buffer_t buf;
	...
	calc_service_destructor(context.svc);
