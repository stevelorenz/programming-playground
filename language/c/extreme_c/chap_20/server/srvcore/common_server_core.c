...
void request_callback(void *obj, struct calc_proto_req_t req) {
	...
	switch (req.method) {
		...
		case ADD:
		case ADDM:
			result = calc_service_add(context->svc, req.operand1, req.operand2,
									  req.method == ADDM);
			break;
		case SUB:
		case SUBM:
			result = calc_service_sub(context->svc, req.operand1, req.operand2,
									  req.method == SUBM);
			break;
		case MUL:
		case MULM:
			result = calc_service_mul(context->svc, req.operand1, req.operand2,
									  req.method == MULM);
			break;
		case DIV: {
			status = calc_service_div(context->svc, req.operand1, req.operand2,
									  &result);
