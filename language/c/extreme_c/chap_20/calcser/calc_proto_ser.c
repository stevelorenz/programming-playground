...
void _parse_req_and_notify(struct calc_proto_ser_t *ser) {
	...
	if (!_parse_fields(ser, fields, FIELD_COUNT_PER_REQ_MESSAGE,
...
void _parse_resp_and_notify(struct calc_proto_ser_t *ser) {
	...
	if (!_parse_fields(ser, fields, FIELD_COUNT_PER_RESP_MESSAGE,
	...
	if (!_parse_int(fields[1], &resp.status)) {
		if (ser->error_cb) {
			ser->error_cb(ser->context, resp.req_id,
...
void calc_proto_ser_delete(struct calc_proto_ser_t *ser) { free(ser); }

void calc_proto_ser_constructor(struct calc_proto_ser_t *ser, void *context,
...
struct buffer_t calc_proto_ser_server_serialize(
	struct calc_proto_ser_t *ser, const struct calc_proto_resp_t *resp) {
	struct buffer_t buff;
	char resp_result_str[64];
	_serialize_double(resp_result_str, resp->result);
	buff.data = (char *)malloc(64 * sizeof(char));
	sprintf(buff.data, "%d%c%d%c%s%c", resp->req_id, FIELD_DELIMITER,
...
struct buffer_t calc_proto_ser_client_serialize(
	struct calc_proto_ser_t *ser, const struct calc_proto_req_t *req) {
	struct buffer_t buff;
	char req_op1_str[64];
	char req_op2_str[64];
	...
	sprintf(buff.data, "%d%c%s%c%s%c%s%c", req->id, FIELD_DELIMITER,
			method_to_str(req->method), FIELD_DELIMITER, req_op1_str,
			FIELD_DELIMITER, req_op2_str, MESSAGE_DELIMITER);
	buff.len = strlen(buff.data);
	return buff;
}
