...
void *mark_factors(void *vin) {
	...
	for (long int scale = 2; si < in->max; scale++, si = scale * in->i) {
		// Protect the critical area with a mutex
...
int main() {
	...
	factor_ct[0] = 0;
	factor_ct[1] = 1;
	for (long int i = 2; i < max; i++) factor_ct[i] = 2;

	one_factor_s x[thread_ct];
	for (long int i = 2; i <= max / 2; i += thread_ct) {
		for (int t = 0; t < thread_ct && t + i <= max / 2;
			 t++) {	 // extra threads do no harm.
			x[t] = (one_factor_s){.i = i + t,
								  .max = max,
								  .factor_ct = factor_ct,
								  .mutexes = mutexes,
								  .mutex_ct = mutex_ct};
		...
		// Join the main thread and wait for all sub threads to finish
		for (int t = 0; t < thread_ct; t++) pthread_join(threads[t], NULL);
	}
	FILE *o = fopen("xpt", "w");
	...
	for (int i = 0; i < thread_ct; i++) {
		thread_info[i] = (tally_s){.this_thread = i,
								   .thread_ct = thread_ct,
								   .tally = tally,
								   .max = max,
								   .factor_ct = factor_ct,
	...
	for (int i = 0; i <= max_factors; i++) printf("%i\t%li\n", i, tally[i]);
}
