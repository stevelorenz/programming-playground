...
int get_max_factors(_Atomic(int) *factor_ct, long int max) {
	...
	for (long int i = 0; i < max; i++) {
		if (factor_ct[i] > global_max) global_max = factor_ct[i];
	}
	return global_max;
}
...
int main(int argc, char *argv[]) {
	...
	for (i = 2; i < max / 2; i += thread_ct) {
		for (t = 0; t < thread_ct && t + i <= max / 2; ++t) {
			x[t] =
				(one_factor_s){.i = i + t, .max = max, .factor_ct = factor_ct};
	...
	for (t = 0; t < thread_ct; ++t) {
		pthread_join(threads[t], NULL);
	}

	for (i = 0; i < max_factors + 1; ++i) {
		printf("%i\t%li\n", i, tally[i]);
	}
