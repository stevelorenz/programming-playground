...
int useHeap(const vector<int> &nums, int k) {
	...
	for (size_t i = 0; i < k - 1; i++) {
		...
		// The actual element is not poped yet, a calling to pop_back on the
		// vector is required to actually pop_back() the element
...
int quickParition(vector<int> &nums, int left, int right) {
	...
	while (l <= r) {
		if (nums[l] < pivot &&
			nums[r] > pivot) {	// Move the larger element to the beginning
			swap(nums[l++], nums[r--]);
		}
		if (nums[l] >= pivot) l++;
		if (nums[r] <= pivot) r--;
...
int useSTLAlgorithm(vector<int> &nums, int k) {
	...
	// It makes sure that all element < nums.end() - k are smaller than the
	// nums[n-k]
